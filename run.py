...
def _solve_init(
    ...
    christoffel: Callable[[ArrayM], ArrayMMM],
    solver: Union[
        optx.AbstractLeastSquaresSolver, optx.AbstractRootFinder, optx.AbstractMinimiser
    ],
    max_steps: None | int,
) -> ArrayM:
...
def solve_bvp(
    ...
    ] = optx.Newton(rtol=1e-4, atol=1e-4),
    ...
    if init_solver is None:
        if init_init_v0s is None:
            init_init_v0s: Float[Array, "pieces M"] = jnp.zeros(
                (pieces, M), target.dtype
            )
        ...
        if init_init_v0s is None:
            init_init_v0s: Float[Array, "pieces M"] = (
                init_x0s[1:] - init_x0s[:-1]
            ) / tdiffs
    ...
    def _root_fn(inputs, _):
        ...
        (x1s, v1s) = eqx.filter_vmap(_solve_single_ivp)(
            t0s,
            t1s,
            x0s,
            v0s,
    ...
    init_inputs = (init_later_x0s, init_v0s)
    sol = optx.root_find(
        _root_fn, opt_solver, init_inputs, max_steps=opt_max_steps
    )
    _, v0s = sol.value
...
def demo_qubit():
    ...
    def inner_product(
        A: Complex[Array, "2*N 2*N"], B: Complex[Array, "2*N 2*N"]
    ) -> Complex[Array, ""]:
    ...
    def metric(x: Float[Array, " M"]) -> lx.AbstractLinearOperator:
        assert x.shape == (M,)
        U: Complex[Array, "2*N 2*N"] = jsp.linalg.expm(
            jnp.einsum("a,abc->bc", 1j * x, σ_vec)
        )
        bb_inner_product = jax.vmap(
            jax.vmap(inner_product, in_axes=(None, 0)), in_axes=(None, 0)
        )
        traces: Complex[Array, "M M"] = bb_inner_product(U, σ_prod)
        matrix: Complex[Array, "M M"] = jnp.einsum(
            "i,ik,im->km", I_diag, traces, traces
        )
    ...
    # TODO: why does the following blow up?
    # _solve_single_ivp(
    #     0.75,
    #     1.0,
    #     jnp.array([0.6909525, 0.60102, 0.0]),
    #     0.98 * jnp.array([1.05247766e00, 9.15490027e-01, 5.39761748e-16]),
    #     christoffel,
    #     dfx.Tsit5(),
    ...
    init_solver = None
    opt_solver = optx.Newton(rtol=1e-4, atol=1e-4)
    diffeq_max_steps = 512
    ...
    for i in range(len(ts)):
        ...
        while not success:
            try:
                v0, optim_num_steps = solve_bvp(
                    target,
                    pieces,
                    christoffel,
                    bounds,
                    init_init_v0s=init_v0s,
                    init_solver=init_solver,
                    opt_solver=opt_solver,
                    diffeq_solver=dfx.Tsit5(scan_kind="bounded"),
                    diffeq_adjoint=dfx.DirectAdjoint(),
                    diffeq_max_steps=diffeq_max_steps,
                )
                if t == 0:
                ...
                vs_saveat = dfx.SubSaveAt(
                    ts=jnp.linspace(0, next_t1, pieces + 1)[:-1], fn=lambda t, y, args: y[0][1]
                )
                ...
                ((x1,), _init_v0s, (length,)), diffeq_num_steps = solve_inference_ivp(
                    v0, metric, christoffel, t1=next_t1, saveat=saveat, max_steps=diffeq_max_steps * pieces
                )
            except (ValueError, jax.lib.xla_extension.XlaRuntimeError):
                # Integration failures may arise from both `solve_bvp` and `solve_inference_ivp`.
                print(f"Failed to integrate {t=:.3f}; reducing initial velocity by 0.9")
                init_v0s = init_v0s * 0.9
            else:
                if t != 0:
                    init_v0s = next_t1 * _init_v0s

                error = (x1 - target).tolist()
                error_string = "[" + ", ".join(f"{x:.5f}" for x in error) + "]"
                print(
                    f"{t=:.3f}, length={length.item():.3f} error={error_string} optim_num_steps={optim_num_steps.item()} diffeq_num_steps={diffeq_num_steps.item()}"
                )
                success = True
...
def demo_torus():
    ...
    v0, _ = solve_bvp(
        ...
        christoffel,
        bounds,
        diffeq_solver=dfx.Tsit5(scan_kind="lax"),
        diffeq_adjoint=dfx.DirectAdjoint(),
    )
    ...
    surface = jnp.mgrid[0.0 : 2 * np.pi : 100j, 0.0 : 2 * np.pi : 100j]
    bbmanifold = jax.vmap(
        jax.vmap(manifold, in_axes=1, out_axes=1), in_axes=1, out_axes=1
    )
    x_surface, y_surface, z_surface = bbmanifold(surface)
    fig = plt.figure()
